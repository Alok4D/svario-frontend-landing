...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "https://svario.ai/api/api/v1";

const rawBaseQuery = fetchBaseQuery({
  baseUrl: API_BASE_URL,
  prepareHeaders: (headers, { getState, endpoint }) => {
    // Public contact form should not send auth header.
    if (endpoint === "createPublicContactSupport") {
